...
def _validate_setting_adherence(
    ...
    for setting_name in _ADHERENCE_SETTINGS:
        ...
        claim = getattr(draft.setting_adherence, setting_name)
        if claim is None:
            raise WorldGenerationError(f"structured world lacks {setting_name} adherence evidence")
        if _normalize_text(claim.confirmed_value) != _normalize_text(str(confirmed)):
            raise WorldGenerationError(f"structured world changed the confirmed {setting_name}")
        ...
        *,
        ...
        if isinstance(requested_concepts, (list, tuple)) and requested_concepts:
            serialized_characters = [
                character.model_dump(mode="json") for character in draft.character_templates
            ]
            if not _requested_concepts_have_distinct_templates(
                [str(item) for item in requested_concepts], serialized_characters
            ):
                raise WorldGenerationError("structured world is missing a requested pregen concept")

        generated_texts = _text_values(draft.model_dump(mode="json", exclude={"setting_adherence"}))
        for constraint in settings.get("content_constraints", ()):
            ...
            if any(
                ...
                raise WorldGenerationError("structured world violates a content boundary")

        _validate_setting_adherence(draft, settings)
